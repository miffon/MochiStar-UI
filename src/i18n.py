...
_ZH_TW = {
    ...
    "Enter a duration such as 00:03:12.500": "輸入時間, 例如 00:03:12.500",
    ...
    "Later": "稍後處理",
    "Close MochiStar, extract the downloaded ZIP file, then replace the old MochiStar folder with the extracted folder. Your settings and queue are stored separately.": "請關閉 MochiStar、解壓縮下載的 ZIP，然後用解壓後的 MochiStar 資料夾取代舊版。設定與佇列資料存放在其他位置，不會被清除。",
    "Close MochiStar, extract the downloaded ZIP file, then drag MochiStar.app to Applications and replace the old version. If macOS blocks it, use Open Anyway in Privacy & Security.": "請關閉 MochiStar、解壓縮下載的 ZIP，然後將 MochiStar.app 拖入「應用程式」並取代舊版。若 macOS 阻擋開啟，請到「隱私權與安全性」選擇「仍要打開」。",
